...
def choose_move(board, color, model):
  ...
  x = torch.Tensor(board_rep).float().to(config.device)
  ...
  for from_ in froms:
    ...
    vals.append(val)
  vals = torch.tensor(vals).to(config.device)
...
# converts a chess board (6x8x8) to chess board from the python chess library
